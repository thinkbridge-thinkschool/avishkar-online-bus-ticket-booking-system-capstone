...
public sealed class PaymentEntityTests
{
    ...
    {
        var payment = Payment.Create(Guid.NewGuid(), 900m, PaymentMethod.UPI);
    ...
    {
        Assert.Throws<ArgumentException>(() => Payment.Create(Guid.NewGuid(), 0m, PaymentMethod.UPI));
    }

    [Fact]
    public void Create_WithNegativeAmount_ShouldThrow()
    {
        Assert.Throws<ArgumentException>(() => Payment.Create(Guid.NewGuid(), -1m, PaymentMethod.CreditCard));
    }

    [Fact]
    public void Complete_ShouldSetCompletedStatus_AndRaiseEvent()
    {
        var payment = Payment.Create(Guid.NewGuid(), 750m, PaymentMethod.CreditCard);
    ...
    {
        var payment = Payment.Create(Guid.NewGuid(), 500m, PaymentMethod.NetBanking);
    ...
    {
        var payment = Payment.Create(Guid.NewGuid(), 500m, PaymentMethod.UPI);
    ...
    {
        var payment = Payment.Create(Guid.NewGuid(), 500m, PaymentMethod.CreditCard);
